...
<html lang="pl" dir="ltr">
  ...
  $names = array(); //tablica dla imion
    foreach ($_SESSION['rodzina'] as $value) {
      echo "<tr>";
          echo "<td>$count</td>";
          //jeżeli takie imie istnieje w tablice names zwiększamy na 1, jeżeli nie to dodajemy do tablicy
          if (array_key_exists($value['name'], $names)) {
            $names[$value['name']]++;
          } else {
            $names[$value['name']] = 1;
          }
          $heightAll += $value['height']; //dodajemy wzrost osoby z rodziny
      foreach ($value as $member) {
        echo "<td>$member</td>"; // echo dane osoby
      }
      $count++; //to jest tylko dla wyprowadzania danych
      echo "</tr>";
    }
   ...
   ?>
    ...
    <br><br>
    <h3>Ilość powtórzonych imion</h3>
    <table>
      <tr>
    <th>Imię</th>
    <th>Ilość</th>
  ...
  <h3>Średni wzrost członków rodziny</h3>
